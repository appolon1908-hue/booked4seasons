...
export function RequestForm({ kind }: { kind: PublicFormKind }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (kind === "service") payload = { kind, ...common, name: `${raw.firstName} ${raw.lastName}`.trim(), email: raw.email, phone: raw.phone,
      ...
      requested_date: raw.preferredDate, requested_timing: raw.preferredTime, contact_preference: "email" };
  ...
  return <form className="request-form" onSubmit={submit} onFocusCapture={start} noValidate={false}>
    ...
    <div className="form-grid">
      ...
      {kind !== "service" && <label>Mobile telephone number<input name="phone" type="tel" required minLength={7} maxLength={40} autoComplete="tel" /></label>}
    ...
    {kind !== "service" && <section className="form-disclosure"><h3>Optional SMS consent</h3><label className="consent-check"><input name="smsConsent" type="checkbox" value="yes" /><span>{smsDisclosure} See our <a href="/sms-terms" target="_blank">SMS Terms</a> and <a href="/privacy" target="_blank">Privacy Policy</a>.</span></label></section>}
