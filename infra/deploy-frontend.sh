...
docker compose --env-file .env.frontend -f docker-compose.frontend.yml build
...
for i in $(seq 1 36); do
  ...
  if [ "$STATUS" = "healthy" ]; then echo "$NEW_SHA" > "$STATE/current-sha"; echo "DEPLOY_SHA=$NEW_SHA"; echo "FRONTEND_HEALTH=PASS"; exit 0; fi
  sleep 5
done
